...
I will start by looking into the "Grave rank symbol" as a symbol of the wealth related to a burial.
...
I will sort out the datapoints with no period assigned to them.
```{r}
...
plot(count_pareto$x, (count_pareto$freq)/length(Iron_age_df$MoundID), type = "l", col = "blue", ylim = c(0,1))
lines(v, pdf_of_pareto)
```
...
plot(count_pareto$x, (count_pareto$freq)/length(transitional_bronze_df$MoundID), type = "l", col = "blue", ylim = c(0,1))
lines(v, pdf_of_pareto)
```
